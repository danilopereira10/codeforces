...
int main() {
    ...
    unordered_map<string,tuple<int,int>> b;
    f.push_back(make_tuple(x,y));
    //f.push_back(y);
    b[to_string(x)+"_"+to_string(y)] = make_tuple(1,0);
    //b[y] = make_tuple(1,1);
    if (n == 1) {
        cout << x EN;
    } else if (n==2) {
        ...
        ll in;
        while (n) {
            ll f2 = (get<1>(f[ind1]) - get<0>(f[ind1])) % (1000000007);
            
            if (f2 < 0) {
                f2 += 1000000007;
            }
            tuple<ll,ll> tf2 = make_tuple(get<1>(f[ind1]), f2);
            
            n--;
            //ind1;
            if (get<0>(b[to_string(get<0>(tf2)) +"_"+to_string(get<1>(tf2))]) != 0) {
                in = get<1>(b[to_string(get<0>(tf2)) +"_"+to_string(get<1>(tf2))]);
                ind1 = in;
                break;
            } else {
                b[to_string(get<0>(tf2)) +"_"+to_string(get<1>(tf2))] 
                        = make_tuple(1, f.size());
                f.push_back(tf2);
                ind1++;
                //ind2++;
        ...
        cout << get<1>(f[in+(n%(f.size()-1-in+1))]) EN;
        //cout << f[(f.size()-1 + n) % f.size()] EN;
