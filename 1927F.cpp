...
//     }
// }
int visit(vector<tuple<int,int>> u2[], int vis[], tuple<int,int> u, tuple<int,int> v, ll w, ll w2, int c[]) {
    ll w3;
    if (vis[get<0>(v)]) {
        return w;
    }
    for (int i = 0; i < u2[get<0>(v)].size(); i++) {

        visit(u2, vis, v, u2[get<0>(v)][i], w, w2, c);
    }
    if (u2[i].si)
    while((get<0>(v)+1) < u2[get<0>(u)].size()) {
...
int main() {
    ...
    for (int i = 0; i < t;i++) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            u--;
            v--;
            u2[u].push_back(make_tuple(v,w));
            u2[v].push_back(make_tuple(u,w));
        }
        ll w = 1e13;
        for (int i = 0; i < n; i++) {
            if (!vis[i]) {
                if (u2[i].size() != 0) {
                    visit(u2, vis, i, u2[i][0], w, w, c);
                }
            }
        }
    }
