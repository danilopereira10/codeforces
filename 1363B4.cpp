...
int main() {
    ...
    for (int i = 0; i < t; i++) {
        ...
        set<tuple<int,list<int>::iterator>> pq;
        list<int>::iterator b = a.begin();
        if (a.size() > 2) {
            b++;
            while (b != a.end()--) {
                pq.insert(make_tuple(*b, b));
                b++;
            }
        }
        ...
        while (a.size() > 2) {
            ...
            // tuple<int, list<int>::iterator> c3 = *pq.begin();
            // if (get<0>(c3) <= (*(c) + *(c2))) {
                // d += get<0>(c3);
                // int & c4(*(get<1>(c3)));
                // c4 += *(get<1>(c3)--);
                // c4 += *(get<1>(c3)++);
                // auto it = pq.find(make_tuple(*(get<1>(c3)--),get<1>(c3)--));
                // pq.erase(it);
                // it = pq.find(make_tuple(*(get<1>(c3)++),get<1>(c3)++));
                // pq.erase(it);
                // a.erase(get<1>(c3)++);
                // a.erase(get<1>(c3)--);
            // } else {
                // list<int>::iterator c4, c5;
                // if (*(c) < *(c2)) {
                //     d += *c;
                //     int & c4(*c);
                //     c4 += *(c++);
                //     auto it = pq.find(make_tuple(*(c++), (c++)));
                //     pq.erase(it);
                //     a.erase((c++));
                // } else {
                //     d += *c2;
                //     int & c4(*c2);
                //     c4 += *(c2--);
                //     auto it = pq.find(make_tuple(*(c2--), (c2--)));
                //     pq.erase(it);
                //     a.erase((c2--));
                // }
