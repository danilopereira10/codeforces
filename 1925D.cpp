...
ll modexp(ll x, ll n)
{
	if (n == 0) {
		...
		return modexp((x * x) % CON, n / 2);
	}
	else {
		return (x * modexp((x * x) % CON, (n - 1) / 2) % CON);
	}
}
...
int main() {
    ...
    for (int i = 0; i < t ;i++) {
        ...
        for (int i = 2; i <= k; i++) {
            ll s1 = (((2*m * 2*(s[i-1]+1)))) % CON;
            ll s2 = (((2*(((n*(n-1))%CON)/2-m)*2*s[i-1]) % CON) % CON) % CON;
            s[i] = (2*((2*m * 2*(s[i-1]+1)) % CON + (2*(((n*(n-1))%CON)/2-m)*2*s[i-1]) % CON) % CON) % CON; 
            d[i] = (((n * (n-1)) % CON) * d[i-1]) % CON;
        }
        ...
        // }
        ll c = n * (n-1);
        cout << getFractionModulo(s[k],d[k]) EN;
