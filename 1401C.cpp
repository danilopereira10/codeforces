...
int main() {
    ...
    for (int i = 0; i < k ; i++) {
        int n;
        cin >> n;
        tuple<ll,int,int> a[n];
        for (int i = 0; i < n; i++) {
            int c;
            cin >> c;
            a[i] = make_tuple(c,i,0);
        }
        sort(a, a+n);
        ll min = get<0>(a[0]);
        
        for (int i = 0; i < n; i++) {
            a[i] = make_tuple(get<0>(a[i]), get<1>(a[i]), i);
            
        }
        sort(a, a+n, [](tuple<ll,int,int> b, tuple<ll,int,int> c) {
        ...
        for (int i = 0; i < n; i++) {
            if (get<1>(a[i]) != get<2>(a[i])) {
                ...
                ll d = get<2>(a[i]);
                if (a[c] != a[d]) {
                    if ((__gcd(get<0>(a[c]),(ll) min) != min) || (__gcd(get<0>(a[d]), min) != min)) {
                        f = 1;
                    }
                }
            }
