...
int main() {
    ...
    for (int i = 0; i < t; i++) {
        ...
        t++;
        //b = b->ant;
        set<tuple<int,p_no>> pq;
        p_no a2 = a->prox;
        if (t > 2) {
            while (a2 != NULL && a2 != b) {
                pq.insert(make_tuple(a2->dado,a2));
                a2=a2->prox;
            }
        }
        d = 0;
        ...
        while (t > 2) {
            tuple<int, p_no> c3 = *pq.begin();
            if (t > 3) {
                if (get<0>(c3) < (a->dado + b->dado)) {
                    d += get<0>(c3);
                    int & c4((get<0>(c3)));
                    c4 += get<1>(c3)->ant->dado;
                    c4 += get<1>(c3)->prox->dado;
                    get<1>(c3)->dado = c4;

                    if (get<1>(c3)->ant != a) {
                        auto it = pq.find(make_tuple(get<1>(c3)->ant->dado,get<1>(c3)->ant));
                        pq.erase(it);
                    } 
                    if (get<1>(c3)->prox != b) {
                        auto it = pq.find(make_tuple(get<1>(c3)->prox->dado, get<1>(c3)->prox));
                        pq.erase(it);
                    }
                    remover_elemento(get<1>(c3)->ant);
                    remover_elemento(get<1>(c3)->prox);
                    if (get<1>(c3)->ant == NULL) {
                        a = get<1>(c3);
                        pq.erase(pq.begin());
                    }else if (get<1>(c3)->prox == NULL) {
                        b = get<1>(c3);
                        pq.erase(pq.begin());
                    } else {
                        p_no c5 = get<1>(c3);
                        pq.erase(pq.begin());
                        pq.insert(make_tuple(c5->dado, c5));
                    }
                    t -= 2;
                } else {
                    if (a->dado < b->dado) {
                        d += a->dado;
                        a->dado += a->prox->dado;
                        auto it = pq.find(make_tuple(a->prox->dado, a->prox));
                        pq.erase(it);
                        remover_elemento(a->prox);
                        t--;
                    } else {
                        d += b->dado;
                        b->dado += b->ant->dado;
                        auto it = pq.find(make_tuple(b->ant->dado, b->ant));
                        pq.erase(it);
                        remover_elemento(b->ant);
                        t--;
                    }
                }
            } else {
                d += min((int)get<0>(c3),(int)min(a->dado,b->dado));
                t--;
            }
        }
        cout << d EN;
