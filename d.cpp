...
int main() {
    ...
    for (int i = k ; i < n; i++) {
        ...
        t = t2;
        t2 += x[i];
        d -= (k*x[i-k]-(t+x[i-k]))*(k*x[i-k]-(t+x[i-k]));
        d += (k*x[i]-t2)*(k*x[i]-t2);
        // d -= 2*(t2-t-x[i-k])*t;
        // d += 2*(t+x[i-k])*(t2-t-x[i-k])*(k-1);
        // d += (k-1)*(t2-t-x[i-k])*(t2-t-x[i-k]);
        d += (t2-t-x[i-k])*(-2)*t*k + (k-1)*(t2*t2-(t+x[i-k])*(t+x[i-k]));
        // d += 2*(t+x[i-k])*(t2-t-x[i-k])*(k-1);
        // d += (k-1)*(t2-t-x[i-k])*(t2-t-x[i-k]);
        if (d < md) {
            md = d;
