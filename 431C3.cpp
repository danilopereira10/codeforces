...
int main() {
    ...
    if (d > n) {
        ...
        for (int j = k+1; j <= n; j++) {
            ll s2 = 0;
            for (int j2 = 1; (j2 <= k) && ((j-j2-1) > -1) && (j2-1 <= j - j2 - 1); j2++) {
                s2 += (ma2[j2-1]*ma2[j-j2-1]) % CON;
            }
            ma2[j-1]=s2;
        }
        ...
        for (int j = d; j <= n; j++) {
            ll s = 0;
            for (int k = 1; k <= d-1 && ((j-k-1) > -1) && (k-1 <= (j-k-1)); k++) {
                s += (ma[k-1]*ma[j-k-1]) % (1000000007);
            }
            ma[j-1] = s;
        }
        ll r = ((ma2[n-1] - ma[n-1]) % 1000000007);
        if (r < 0) {
            r+= CON;
        }
        cout << r EN;
